...
export const useDeleteRecipeMutation = (id) => {
  ...
  return useMutation({
    ...
    onError: (err) => {
      toast.error(err.response?.data?.message || "Не удалось удалить рецепт");
    },
  });
};
...
export const recipeService = {
  getRecipes: async (params = {}) => {
    ...
    const processedParams = {
      ...
      tags:
        Array.isArray(params.tags) && params.tags.length > 0
          ...
          : undefined,
      cuisines:
        Array.isArray(params.cuisines) && params.cuisines.length > 0
    ...
    Object.keys(processedParams).forEach(
      (key) =>
        processedParams[key] === undefined && delete processedParams[key],
    );
